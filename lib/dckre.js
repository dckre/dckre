...
class Processor {
  constructor(dckrefile) {
    ...
    $fsp
      ...
      .filter((instruction) => {
        return !(instruction.name === 'ENV' && Object.keys(instruction.args).length < 1)
      })
      .map((instruction) => {
        if (instruction.name === 'ENV') {
          let envs = Object.keys(instruction.args).map((key) => {
            return key + '=' + instruction.args[key]
          })
          return 'ENV ' + envs.join(' ')
        }
  ...
  include(args) {
    ...
    // Enforce ".dckre" suffix to enable uniform standard
    file = file + '.dckre'
